...
class CompleteTool(BaseModel):
    ...
    answer: str = Field(description="Plugin's response, if json string is the response copy it as it is don't cut or add something.")
...
class FinalResponse(BaseModel):
    ...
    structured_answers: Union[List[Union[str, dict]], None] = Field(description="All json string answers from `CompletedTool`s should be copied here if they are json.")
    text_answer: Union[str, None] = Field(description="Union of tool text answers from `CompleteTool`s tools which wasn't they aren't json.")

    class Config:
        schema_extra = {
            "example": {
                "structured_answers": ['"data": {"sol": 12, "usdc": 10}}', '"data": {"sol": 12, "usdc": 10}'],
                "text_answer": "Your transaction successfully completed and you also successfully staked 10 sol on kamino."
            },
            "example 2": {
                "text_answer": "Solana looks very decent to be bought."
            },
            "example 3": {
                "structured_answer": ['"data": {"btc": {"score": 1', '"eth": {"score": 0}}']
            }
        }
