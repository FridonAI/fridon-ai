...
@Registry('kamino-supply')
export class KaminoSupply extends InterfaceSnippet<Request, Response> {
  private logger = new Logger(KaminoSupply.name);

  async execute(data: Request): Promise<Response> {
    this.logger.log(`Executing action: ${KaminoSupply.name}`);
    ...
    const tx = await this.kaminoFactory.supply(
      walletAddress,
      mintAddress,
      amount,
    );
