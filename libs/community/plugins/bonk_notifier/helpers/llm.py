...
def get_bonk_bullish_notification_chain() -> ChatPromptTemplate:
    ...
    check_propmt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are expert in generating notifications for bullish coins.""",
            ),
            (
                "human",
                """Given the metadata of notification generate notification as short text, maximum 3 sentences.""",
            ),
            ("human", "Notification: \n\n {description}"),
        ]
    )
