...
class CoinPaprikaCoinPriceDataProvider(CoinPriceDataProvider):
    ...
    async def _fetch_coin_ohlcv_binance(self, symbol, interval, days=None, limit=1, start_time=None, end_time=None):
        if days is not None:
            ...
            url = f"{self.base_url}/{self.coins_symbols_to_ids[symbol]}&interval={interval}&start={start_time}&end={end_time}"
        else:
            url = f"{self.base_url}/{self.coins_symbols_to_ids[symbol]}&interval={interval}&end={end_time}&limit={limit}"
